...
description: Evaluate wiki claims for fidelity and supersession. Falsification-first posture. Requires Plan 5 CLI tools (lacuna claims, lacuna adversary-commit).
---
...
**Prerequisites:** `lacuna claims` and `lacuna adversary-commit` must be available (Plan 5). The daemon must be running.
